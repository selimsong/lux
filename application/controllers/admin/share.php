...
class Share extends CI_Controller {
	...
	public function index(){
		...
		$getwhere = array();
		$data = $this->Data_model->getData($getwhere,'',0,100,'share_record');
		$res = array(
				'tpl'=>'list',
	...
	function _setlist($data,$ismultiple=true){
		...
		$provinceList = $this->Data_model->getData('', '', '', '', 'province');
		$province = array();
		foreach ($provinceList as $k => $v){
			$province[$v['id']] = $v['province'];
		}
		$cityList = $this->Data_model->getData('', '', '', '', 'city');
		$city = array();
...
//			}
			...
			<td width=80 align="left">'.$item['nickname'].'</td>
			<td width=480 align="left">'.$item['content'].'</td>
			<td width=80 align="left">'.$province[$item['province']].'</td>
			<td width=80 align="left">'.$city[$item['city']].'</td>
